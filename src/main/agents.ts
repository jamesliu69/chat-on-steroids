...
/**
 * Sleep a detached worker after its browser view and its server-side work have both gone quiet.
 *
 * An attached worker may sit on a long tool-free native generation without publishing fresh
 * recorder activity. Silence alone therefore cannot release its slot; the bridge's durable
 * quiescence proof owns that decision. A detached worker has no page left to supply that proof,
 * so its exact calls can renew this bounded escape hatch until the server-side work also stops.
 */
export function sleepSilentWorkers(now = Date.now(), runId?: string, isWorking: (conversationId: string) => boolean = () => false): FinishResult[] {
  ...
  for (const agent of [...runs.values()].filter(run => runId === undefined || run.runId === runId).flatMap(run => [...run.agents.values()])) {
    if (agent.info.role !== 'worker' || agent.info.state !== 'detached' || !agent.info.conversationId) continue;
