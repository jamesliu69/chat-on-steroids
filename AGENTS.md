...
Only detached workers use `WORKER_SILENCE_MS` (three minutes) as a bounded escape hatch after
their browser view disappears. Exact server-side work renews that clock, and the clock starts no
earlier than detachment. An attached active worker is never slept from wall-clock silence alone:
long tool-free native generation can remain valid without publishing fresh recorder activity, so
its slot releases only from durable terminal/quiescence proof. A currently running tool protects
its exact detached worker; another chat's or unidentified request cannot hold all worker slots.
An attached Worker whose exact one-shot silence recovery was confirmed may also release after the
full post-reload window expires with no new work; the confirmed repair receipt, not inactivity
alone, is the authority for that transition.
