...
permission after the await, and cut with an in-context instruction to read the remainder. If even
the AGENTS framing and cut notice cannot fit, delivery fails visibly instead of silently omitting
...
capabilities. Revalidate that directory immediately before an external open action; a replaced,
linked or unavailable root must fail rather than opening its new target. Reading or writing it
never changes the chat's learned project workspace.
...
leading command block, once per selected id; a blank separator or any non-command line ends the
block. Main preparation receives authored input before
...
English, Traditional Chinese and Simplified Chinese are explicit UI translations (`i18n.ts`,
`locales/zh-TW.json`, `locales/zh-CN.json`), with Traditional Chinese as the first-launch
default and the selected locale in `cos.ui.language`. Existing `zh-CN` preferences remain
Simplified Chinese; they are not migrated. New UI copy must be present in both Chinese catalogs.
Changing language repaints owned labels while retaining drafts/selections; never translate
authored messages, provider text or file paths. Bindings live only in a WeakMap keyed by their
DOM node. Language changes walk the current document, including hidden panels and bound text
nodes. Never retain or periodically dereference an index of every past label: WeakRef sweeps keep
detached trees alive during allocation-heavy repaints. `scripts/verify-renderer-label-memory.cjs`
checks real Chromium collection under repeated row replacement; ordinary language tests preserve
controls, drafts and authored values. Authored prose uses automatic text direction; shell/code
remain LTR with logical layout edges. Theme and layout preferences do not change backend
authority. Settings places ChatGPT model defaults second and Workers & recovery third, after
Continuation sources; Appearance and its language selector come last. The connector-instructions
editor is removed. Settings saves preserve existing stored MCP instructions for compatibility.
...
Refused cached calls explain a uniquely known plugin's disabled, sign-in or server-error state.
Connector refresh cannot repair those states. Retained declarations explain refusal only;
they never route a call or resolve conflicting owners. Refusals state no call was dispatched.
