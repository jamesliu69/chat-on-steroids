...
it('uses the same localized approval guidance in Setup and the popup', async () => {
  ...
  expect(document.querySelector('.chatgpt-permission-setup-notice h3')!.textContent).toBe('重要：ChatGPT 可能正在等待您的核准');
  expect(document.getElementById('chatgptPermissionNoticeTitle')!.textContent).toBe('重要：ChatGPT 可能正在等待您的核准');
  expect(document.getElementById('chatgptPermissionUnderstand')!.textContent).toBe('知道了');
  expect(document.querySelector('.chatgpt-permission-setup-notice figcaption')!.textContent).toBe('示意圖。ChatGPT 的實際介面可能有所不同。');
});
