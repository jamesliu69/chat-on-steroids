...
describe('unattributed activity recovery', () => {
  ...
  it('projects Pro silence from the real work clock and renews the hidden first five minutes on activity', async () => {
    ...
    try {
      ...
      expect(repair?.reason).toBe('silence');
      await maintenance(repair!.token);
      expect((await sessionControlsFor(id)).recovery).toEqual([{ kind: 'post-reload', deadline: Date.now() + 300_000 }]);
    } finally { vi.useRealTimers(); }
  });
  ...
  it('queues the silence reload on the deadline without waiting for the maintenance tick', async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(2);
      // The deadline callback revalidates durable source-turn MCP proof asynchronously.
  ...
  it.each([
    ...
    try {
      ...
      const listenUntil = confirmedAt + (pro ? 300_000 : 60_000);
      expect(goalPendingReplyFor(OTHER)?.listenUntil).toBe(listenUntil);
      const sessionId = (await findSessionByConversation(OTHER, { requireUnique: true }))!.id;
      if (Date.now() < listenUntil) {
        expect((await sessionControlsFor(sessionId)).recovery).toEqual([{ kind: 'post-reload', next: 'loop', deadline: listenUntil }]);
        expect((await request('POST', '/goal/draft', { body: { conversationId: OTHER, turnId: pending.turnId } })).body.error).toBe('chat_still_working');
        await vi.advanceTimersByTimeAsync(listenUntil - Date.now() - 1);
        expect((await request('POST', '/goal/draft', { body: { conversationId: OTHER, turnId: pending.turnId } })).body.error).toBe('chat_still_working');
        await vi.advanceTimersByTimeAsync(1);
      }
      await sweepStaleSwarm(Date.now());
  ...
  it('files one opted-in Pro Loop ticket on refresh, defers native busy, and revokes it on MCP work', async () => {
    ...
    try {
      ...
      expect((await sessionControlsFor(activity.sessionId)).recovery).toEqual([{ kind: 'post-reload', next: 'loop', deadline: deferred.listenUntil }]);
